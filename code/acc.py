...
def PSNR(y, x): #y_true, y_pred for 3d input
    ...
    for i in range(AMT):
        ...
        y_tmp = tf.dtypes.cast(y_tmp,tf.uint8)
        
        x_tmp = x[i, :, :].reshape((HT, WDTH))
        x_tmp = tf.dtypes.cast(x_tmp,tf.uint8) 
        
        (x_tmp, y_tmp) = to_ndarray(x_tmp, y_tmp)
        y_tmp = y_tmp.reshape((HT, WDTH, 1))
        x_tmp = x_tmp.reshape((HT, WDTH, 1))
        y_tmp = tf.convert_to_tensor(y_tmp, dtype = tf.uint8)
        x_tmp = tf.convert_to_tensor(x_tmp, dtype = tf.uint8)
        
        PSNR_sum += tf.Session().run(tf.image.psnr(x_tmp, y_tmp, max_val=255))
...
def SSIM(y, x):
    ...
    for i in range(AMT):
        ...
        ssim_sum += tf.Session().run(tf.image.ssim(x_tmp, y_tmp, max_val=255))    #Image.fromarray(x_tmp), Image.fromarray(y_tmp)
