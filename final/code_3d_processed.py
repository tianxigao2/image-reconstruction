...
def SRnet_3d_model(AMOUNT, DEPTH, TARGET_HEIGHT, TARGET_WIDTH):
    ...
    model.add(Conv3D(8, kernel_size = (1, 1, 3), strides=(1, 1, 1), padding='same', dilation_rate=(1, 1, 1), activation = 'relu', use_bias=True, kernel_initializer='glorot_uniform', bias_initializer='zeros', kernel_regularizer=None, bias_regularizer=None, activity_regularizer=None, kernel_constraint=None, bias_constraint=None))
    
    model.add(Conv3D(8, kernel_size = (3, 3, 3), strides=(1, 1, 1), padding='same', dilation_rate=(1, 1, 1), activation = 'relu', use_bias=True, kernel_initializer='glorot_uniform', bias_initializer='zeros', kernel_regularizer=None, bias_regularizer=None, activity_regularizer=None, kernel_constraint=None, bias_constraint=None))
    
    model.add(Conv3D(8, kernel_size = (3, 3, 3), strides=(1, 1, 1), padding='same', dilation_rate=(1, 1, 1), activation = 'relu', use_bias=True, kernel_initializer='glorot_uniform', bias_initializer='zeros', kernel_regularizer=None, bias_regularizer=None, activity_regularizer=None, kernel_constraint=None, bias_constraint=None))
    
    model.add(Conv3D(32, kernel_size = (1, 1, 3), strides=(1, 1, 1), padding='same', dilation_rate=(1, 1, 1), activation = 'relu', use_bias=True, kernel_initializer='glorot_uniform', bias_initializer='zeros', kernel_regularizer=None, bias_regularizer=None, activity_regularizer=None, kernel_constraint=None, bias_constraint=None))
...
def three_d_processed(train_y, test_y, AMOUNT, AMOUNT_TEST, DEPTH, HEIGHT, WIDTH, TARGET_HEIGHT, TARGET_WIDTH):
    
    #load train set and test set x for neural network--------------------------------------------------
    f_train = h5py.File('interim_result_trained_by_fsrcnn/climate_data_fsrcnn_result_train.h5', 'r')
    train_x = numpy.array(f_train.get('fsrcnn_processed'))
    f_test = h5py.File('interim_result_trained_by_fsrcnn/climate_data_fsrcnn_result_test.h5', 'r')
    test_x = numpy.array(f_test.get('fsrcnn_processed'))
    
    plt.imshow(test_x[0, :, :, 0])
    plt.title('reloaded raw test_x')
    plt.savefig('three_d_processed/raw_test_3d.png')
    ...
    plt.savefig('three_d_processed/packed_test_3d.png')
    ...
    callbacks=[EarlyStopping(monitor='val_loss', patience=8, verbose=2, mode='auto', baseline=None, restore_best_weights=False),
               ModelCheckpoint(filepath = './three_d_processed/3d_model_climate_300.h5', monitor='val_loss', verbose=2, save_best_only=True, save_weights_only=False, mode='auto', period=1)])
    ...
    plt.savefig('three_d_processed/final_test_3d.png')
    plt.imshow(test_y[0, :, :, 0])
    plt.title('test_y')
    plt.savefig('three_d_processed/test_y_3d.png')
